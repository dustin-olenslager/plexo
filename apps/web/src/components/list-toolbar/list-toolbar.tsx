                        ...
                        {options.map((opt) => {
                            ...
                            return (
                                <li key={opt.value} role="option" aria-selected={active}>
                                    <button
                                        type="button"
                                        className={`w-full flex items-center justify-between gap-3 rounded-md px-3 py-3 md:px-2.5 md:py-1.5 text-[16px] md:text-sm transition-colors text-left ${active ? 'bg-azure/15 text-azure-600' : 'text-text-secondary hover:bg-surface-2 hover:text-text-primary'
                                            }`}
                                        onClick={() => {
                                            onChange(opt.value)
                                            setOpen(false)
            ...
            <div className="flex flex-col md:flex-row gap-2">
                ...
                <div className="relative flex-1">
                    ...
                    <input
                        ...
                        className="w-full rounded-lg border border-border bg-surface-1/60 py-2 pl-10 pr-4 min-h-[44px] md:min-h-[32px] text-[16px] md:text-sm text-text-primary placeholder:text-text-muted focus:border-azure focus:outline-none focus:ring-1 focus:ring-azure/20 transition-colors"
                    />
                    {search && (
                        <button
                            onClick={() => setSearch('')}
                ...
                {hasDimensions && (
                    ...
                    >
                        ...
                        Filters
                        {activeFilterCount > 0 && (
                            <span className="ml-0.5 rounded-full bg-azure px-1.5 py-0.5 text-[9px] font-semibold text-white leading-none">
                                {activeFilterCount}
                            </span>
                        )}
                    </button>
                    ...
                    {activeDims.map((dim) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <button
                                    onClick={() => setFilter(dim.key, null)}
                                    className="hover:text-azure/70 ml-0.5 h-6 w-6 md:h-4 md:w-4 flex items-center justify-center shrink-0"
