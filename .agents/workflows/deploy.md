...
```bash
ssh -o StrictHostKeyChecking=no root@VPS_HOST "cd /opt/plexo && git pull origin main && docker compose -f docker/compose.yml build <service> && docker compose -f docker/compose.yml up -d <service>"
```
